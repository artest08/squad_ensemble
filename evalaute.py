...
model_name1 = "deepset/electra-base-squad2"
model_name2 = "twmkn9/bert-base-uncased-squad2"
...
model1.to(device)
model2.to(device)
constant1 = torch.tensor([0.7]).to(device)
constant2 = torch.tensor([0.3]).to(device)
for example in examples:
    ...
    ##MAx
    start_logits_cat = torch.cat((output1.start_logits, output2.start_logits), 0)
    end_logits_cat = torch.cat((output1.end_logits, output2.end_logits), 0)

    start_logits = torch.max(start_logits_cat, 0, keepdim=True)[0]
    end_logits = torch.max(end_logits_cat, 0, keepdim=True)[0]

    start_logits = torch.min(start_logits_cat, 0, keepdim=True)[0]
    end_logits = torch.min(end_logits_cat, 0, keepdim=True)[0]

    #Summation

    # start_logits = (constant1 * output1.start_logits + constant2 * output2.start_logits)
    # end_logits = (constant1 * output1.end_logits + constant2 * output2.end_logits)
